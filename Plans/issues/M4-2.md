# M4-2 · Add screen-level flow tests for onboarding, capture, and studio

Milestone: M4 QA · Labels: qa, coverage, ux, M4
...
- **D29.1 — onboarding camera-feedback is one source of truth, not two.**
  ...
  imports the pure branch from `src/onboarding/cameraState.ts` (the file the M4-2 tests exercise).
...
Gate-green (tsc + eslint exit 0 + jest 129 tests / 15 suites). M4-2 contributes **39 tests across 5
new suites** (onboarding × 2, capture, wardrobe, composer-notice); the full run also picks up the
concurrent M4-3 / M4-4 suites. The onboarding duplication (D29.1) was fixed and the four screens now
...
| `jest` | ✅ PASS (129 tests / 15 suites — M4-2 adds 39 across 5 new suites: `onboarding/cameraState`, `onboarding/entryRedirect`, `capture/draft`, `wardrobe/emptyState`, `composer/notice`) |
