...
export default function LooksScreen() {
     ...
     const [notice, setNotice] = useState<string | null>(null);
      ...
      const reShare = useCallback(async (look: TryOn) => {
          const result = await shareLook(look);
          if (result.kind === 'error') setNotice(`Share failed: ${result.error}`);
          else setNotice('Look shared.');
       }, []);
      ...
      return (
            <View style={styles.screen}>
               <Text style={styles.header}>Your looks</Text>
               {notice ? <Text style={styles.notice}>{notice}</Text> : null}

               {!looks.length && !loading ? (
                    <Text style={styles.empty}>No looks yet. Try on something in the wardrobe.</Text>
                ) : null}

               {loading && !looks.length ? <Text style={styles.empty}>Loading…</Text> : null}
                     ...
                     <View style={styles.grid}>
                        {looks.map((look) => (
                            <TouchableOpacity
                              ...
                              onPress={() => reopen(look)}>
                                <Image
                                  source={(look.outputPath ? { uri: look.outputPath } : sampleBody) as ImageSourcePropType}
                                  style={styles.thumb}
                                  resizeMode="cover"
                                  fadeDuration={0}
                                 />
                                 <Text style={styles.caption} numberOfLines={1}>
                                   Look #{look.id} · {look.createdAt.slice(0, 10)}
                                 </Text>
                                 ...
                                 <Text style={[styles.chipText, styles.chipDanger]}>Delete</Text>
                                   </TouchableOpacity>
                                  </View>
                              </TouchableOpacity>
                        ))}
