...
const CATEGORIES: ItemCategory[] = ['top', 'bottom', 'dress', 'outerwear', 'shoes', 'other'];
   ...
   if (loading || !item) {
       return (
            <View style={styles.center}>
               <Text style={styles.hint}>{loading ? 'Loading…' : 'Item not found.'}</Text>
               {!loading ? (
         ...
         <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
           ...
           <View style={styles.chips}>
              {CATEGORIES.map((c) => (
                     <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                      onPress={() => setType(c)}>
                       <Text style={[styles.chipText, c === type ? styles.chipTextActive : null]}>{c}</Text>
                     </TouchableOpacity>
                 ))}
           </View>
           ...
           <View style={styles.chips}>
             {COLORS.map((c) => (
                 <TouchableOpacity
                  key={c.name}
                  style={[styles.swatch, { backgroundColor: c.swatch }, c.name === color ? styles.swatchActive : null]}
