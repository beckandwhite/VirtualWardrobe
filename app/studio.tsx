...
function resolveGarmentSource(item: Garment): ImageSourcePropType {
  const real =
    item.imagePath && !item.imagePath.startsWith(CATALOG_PLACEHOLDER);
...
export default function StudioScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setManualBanner((b) =>
        b.dismissed ? b : { open: !placed, dismissed: b.dismissed },
      );
    })();
    return () => {
      alive = false;
  ...
  const showSkeletonWithPoints =
    showSkeleton && keypoints.length > 0 && !!garment;
  ...
  const statusText = autoPlaced
    ? t('studio.status.autoPlaced')
    : t('studio.status.manual');
    ...
    <View style={styles.screen}>
      ...
      {manualBanner.open && !manualBanner.dismissed && (
        <View style={styles.banner}>
          <Text style={styles.bannerText}>{t('studio.banner.manual')}</Text>
          <TouchableOpacity
            onPress={() => setManualBanner((b) => ({ ...b, dismissed: true }))}
          >
            <Text style={styles.bannerDismiss}>{t('common.dismiss')}</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      <View style={styles.controls}>
        ...
        >
          <Text style={styles.primaryText}>
            {saving ? t('common.saving') : t('studio.saveShare')}
          </Text>
        </TouchableOpacity>
      </View>
